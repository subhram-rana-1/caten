...
class OpenAIService:
    ...
    async def get_word_explanation(self, word: str, context: str, language_code: Optional[str] = None) -> Dict[str, Any]:
        ...
        Args:
            ...
            language_code: Optional target language code. The response will be in the detected language of the context,
                          then translated to this language_code if provided.
        """
        try:
            # Detect the language of the context (not use languageCode in prompt)
            ...
            # Always prompt OpenAI to respond in the detected language
            ...
            try:
                ...
                # Translate to target language if languageCode is provided and different from detected language
                if language_code and language_code.upper() != detected_language_code.upper():
                    logger.info("Translating word explanation to target language", 
                               detected_language=detected_language_code, 
                               target_language=language_code)
                    texts_to_translate = [explanation_data['meaning']] + explanation_data['examples']
                    translated_texts = await self.translate_texts(texts_to_translate, language_code.upper())
                    explanation_data['meaning'] = translated_texts[0]
                    explanation_data['examples'] = translated_texts[1:3]
    ...
    async def simplify_text_stream(self, text: str, previous_simplified_texts: List[str], language_code: Optional[str] = None):
        ...
        Yields chunks of simplified text. The response is generated in the detected language of the input text,
        then translated to language_code if provided.

        Args:
            text: The text to simplify
            previous_simplified_texts: Previous simplified versions for context
            language_code: Optional target language code. The response will be in the detected language of the text,
                          then translated to this language_code if provided.
        """
        try:
            # Detect the language of the text (not use languageCode in prompt)
            ...
            # Accumulate the full response in detected language
            accumulated_text = ""
            ...
            # Translate to target language if languageCode is provided and different from detected language
            if language_code and language_code.upper() != detected_language_code.upper():
                logger.info("Translating simplified text to target language",
                           detected_language=detected_language_code,
                           target_language=language_code)
                translated_texts = await self.translate_texts([accumulated_text], language_code.upper())
                accumulated_text = translated_texts[0] if translated_texts else accumulated_text

            # Stream the final text (translated if needed) character by character or in chunks
            # For better UX, we'll yield it in chunks of reasonable size
            chunk_size = 50
            for i in range(0, len(accumulated_text), chunk_size):
                yield accumulated_text[i:i + chunk_size]
...
{language_requirement}"""
    ...
    async def generate_contextual_answer_stream(self, question: str, chat_history: List, initial_context: Optional[str] = None, language_code: Optional[str] = None):
        ...
        Yields chunks of text. The response is generated in the detected language of the question/context,
        then translated to language_code if provided.
        ...
        Args:
            ...
            language_code: Optional target language code. The response will be in the detected language of the question/context,
                          then translated to this language_code if provided.
        """
        try:
            # Detect the language from question, initial_context, and recent chat history
            # Combine all text to detect the primary language
...
{language_requirement}"""
            ...
            # Translate to target language if languageCode is provided and different from detected language
            if language_code and language_code.upper() != detected_language_code.upper():
                logger.info("Translating contextual answer to target language",
                           detected_language=detected_language_code,
                           target_language=language_code)
                translated_texts = await self.translate_texts([accumulated_answer], language_code.upper())
                accumulated_answer = translated_texts[0] if translated_texts else accumulated_answer

            # Stream the final answer (translated if needed) in chunks
            chunk_size = 50
            for i in range(0, len(accumulated_answer), chunk_size):
                yield accumulated_answer[i:i + chunk_size]
...
Summary:"""
    ...
    async def summarise_text_stream(self, text: str, language_code: Optional[str] = None):
        ...
        Yields:
            Chunks of the summary text. The response is generated in the detected language of the input text,
            then translated to language_code if provided.
        """
        try:
            # Detect the language of the text (not use languageCode in prompt)
...
CRITICAL LANGUAGE REQUIREMENT:
- You MUST respond STRICTLY in {detected_language_name or detected_language_code} ({detected_language_code})
- The summary MUST be in {detected_language_name or detected_language_code} ONLY
- Do NOT use any other language - ONLY {detected_language_name or detected_language_code}
- This is MANDATORY and NON-NEGOTIABLE
...
Summary:"""
            ...
            # Translate to target language if languageCode is provided and different from detected language
            if language_code and language_code.upper() != detected_language_code.upper():
                logger.info("Translating summary to target language",
                           detected_language=detected_language_code,
                           target_language=language_code)
                translated_texts = await self.translate_texts([accumulated_summary], language_code.upper())
                accumulated_summary = translated_texts[0] if translated_texts else accumulated_summary

            # Stream the final summary (translated if needed) in chunks
            chunk_size = 50
            for i in range(0, len(accumulated_summary), chunk_size):
                yield accumulated_summary[i:i + chunk_size]
